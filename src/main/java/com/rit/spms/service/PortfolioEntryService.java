...
public class PortfolioEntryService {
    ...
    public PortfolioEntry logAchievementWithEvaluation(Long measurementId, CreateAchievementRequest achievementReq,
        ...
        // recordAchievement above already blocked this entirely once the cycle is
        // EMPLOYEE_SUBMITTED/HEAD_SUBMITTED/CONCLUDED -- the one case still allowed post-submission
        // is RETURNED_TO_EMPLOYEE, and that's exactly when the head should be told a new achievement
        // showed up on a portfolio they're mid-review-round on.
        achievementService.findEvaluation(achievement)
                .filter(e -> e.getState() == AnnualEvaluationState.RETURNED_TO_EMPLOYEE)
                .ifPresent(e -> eventPublisher.publishEvent(
                        new AnnualEvaluationAchievementAddedEvent(e.getId(), achievement.getTitle())));
    ...
    public PortfolioEntry updateEntry(Long entryId, Long criteriaId, Integer categoryRating, Long goalId, String evidenceUrl, Long currentUserId) {
        ...
        if (achievementService.isSubmittedForReview(entry.getAchievement())) {
    ...
    /** Author, or anyone up the author's org hierarchy (their department head, or any org-group
     *  head above that department, transitively -- the exact same definition
     *  AnnualEvaluationService.assertCanView already uses for full evaluation visibility, reused
     *  here instead of re-deriving a narrower "assigned rater for this one year" check). Ordinary
     *  strategy members are deliberately excluded: the achievement itself is visible to them, but
     *  its self-assessment rating, evidence link, and Annual Goal are personal to the employee. */
    private boolean isAuthorOrInHierarchyAbove(Achievement achievement, Long currentUserId) {
        AppUser author = achievement.getAuthor();
        if (author.getId().equals(currentUserId)) {
            return true;
        }
        Department dept = author.getDepartment();
        return dept != null && permissionService.resolveHierarchyDepartmentIds(currentUserId).contains(dept.getId());
    }
     ...
     *  have no measurement at all -- there's no strategy to check read-access against, so only the
     *  achievement's own author or someone in their org hierarchy may look it up. */
    public PortfolioEntry getEntryByAchievementId(Long achievementId, Long currentUserId) {
        ...
        if (achievement.getMeasurement() != null) {
        ...
        } else if (!isAuthorOrInHierarchyAbove(achievement, currentUserId)) {
            throw new UnauthorizedException("You do not have access to this achievement");
        }
        return entryRepository.findByAchievementId(achievementId).orElse(null);
    }

    /**
     * Whether `currentUserId` may see an achievement's self-assessment rating, evidence link, and
     * which Annual Goal it's linked to -- the achievement's own author always can; otherwise only
     * someone in their org hierarchy (their head, or any head above that head). These are personal
     * to the employee, so this is stricter than ordinary Strategy read-access -- any strategy
     * member can see the achievement itself, but not this evaluation-facing detail.
     *
     * <p>A strategy Owner is a third exception: {@link com.rit.spms.service.AchievementService
     * #updateAchievement} already lets an Owner edit any achievement in their strategy regardless
     * of org hierarchy, and that edit form is prefilled from this same entry -- redacting these
     * fields from them would silently blank out (and, since evidence is required, block saving)
     * an edit they're otherwise fully entitled to make.
     */
    public boolean canViewSensitiveFields(Long achievementId, Long currentUserId) {
        Achievement achievement = achievementRepository.findById(achievementId)
                .orElseThrow(() -> new ResourceNotFoundException("Achievement", achievementId));
        if (isAuthorOrInHierarchyAbove(achievement, currentUserId)) {
            return true;
        }
        if (achievement.getMeasurement() == null) {
            return false;
        }
        Long strategyId = achievement.getMeasurement().getInitiative().getObjective().getGoal().getStrategy().getId();
        return permissionService.isOwner(currentUserId, strategyId);
    }
    ...
    public PortfolioEntry upsertEvaluationForAchievement(Long achievementId, Long categoryId, Long criteriaId, Integer categoryRating,
        ...
        if (achievementService.isSubmittedForReview(achievement)) {
            throw new BusinessRuleException(
                    "This achievement has been submitted for annual evaluation review and can no longer be edited.");
        }

        boolean isAuthor = achievement.getAuthor().getId().equals(currentUserId);
        // Achievements generated by a criterion-assigned achievement tool have no measurement, and
        // so no strategy/Owner concept -- only their own author can ever touch them.
        boolean isOwner = achievement.getMeasurement() != null && permissionService.isOwner(currentUserId,
                achievement.getMeasurement().getInitiative().getObjective().getGoal().getStrategy().getId());
        if (!isAuthor && !isOwner) {
        ...
        PortfolioEntry entry = entryRepository.findByAchievementId(achievementId).orElse(null);
        // An achievement-recording tool's category/criteria are fixed for the lifetime of the
        // entry (same rule the annual evaluation's own designation columns already enforce) --
        // ignore whatever this request asked for and keep what's already there.
        boolean categoryCriteriaLocked = achievement.getCreatedByModuleCode() != null && entry != null;
        if (entry == null) {
            entry = PortfolioEntry.builder().achievement(achievement).employee(achievement.getAuthor()).build();
        }
        if (!categoryCriteriaLocked) {
